    ...
    <div className="mx-auto w-full max-w-[1800px] space-y-6 overflow-x-hidden">
      ...
      {(currentRole === "super_admin" || currentRole === "admin") && cfConfigured && (
        <section className="print:hidden">
          ...
          {/* User email sync status */}
          {(() => {
            const withEmail = pins.filter((p) => p.email && !p.locked);
            const noEmail = pins.filter((p) => !p.email && !p.locked);
            const locked = pins.filter((p) => p.locked && p.email);
            return (
              <div className="mt-3 rounded-lg border border-white/5 bg-gray-950/50 px-4 py-3">
                ...
                {withEmail.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {withEmail.map((p) => (
                      <span key={p.id} className="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/10 px-2.5 py-1 text-xs text-emerald-300">
                ...
                {locked.length > 0 && (
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {locked.map((p) => (
                      <span key={p.id} className="inline-flex items-center gap-1.5 rounded-full bg-gray-500/10 px-2.5 py-1 text-xs text-gray-500 line-through">
