...
const DATA_SOURCES = [
  {
    ...
    description: "Your staff exports an Excel file from the current software and uploads it here. Works immediately with zero installation.",
    pros: ["No changes to your existing system", "Staff already knows how to export", "Start seeing real data in minutes"],
    cons: ["Manual step — staff needs to export daily", "Not real-time — data is as fresh as the last upload"],
    frequency: "Once or twice a day",
    effort: "Zero installation. Just upload.",
  },
  {
    id: "bridge",
    ...
    description: "A small read-only program runs on your Dubai server. It reads your database every 15-30 minutes and sends the data to this dashboard automatically. Like a CCTV camera for your data — sees everything, touches nothing.",
    pros: ["Fully automatic — no daily effort from staff", "Near real-time data (every 15-30 min)", "Read-only — your data is never modified", "Once set up, it just works forever"],
    cons: ["Requires one-time installation on your Dubai server", "Need to understand your current software's database"],
    frequency: "Every 15-30 minutes (automatic)",
    effort: "One-time 2-3 hour setup on your server.",
  },
  {
    id: "export",
    ...
    description: "Your current software exports data automatically on a schedule (every 30 min or hourly) to a folder. Our system picks it up and imports it — no manual work needed.",
    pros: ["No manual effort after setup", "Your software does the export automatically", "Data refreshes on a schedule"],
    cons: ["Your software must support scheduled/automatic export", "Not instant — depends on export schedule", "Requires setting up the auto-export in your software"],
    frequency: "Every 30-60 minutes (scheduled)",
    effort: "Configure export schedule in your software + one-time setup on server.",
  },
  {
    id: "vpn",
    name: "VPN Access",
    ...
    description: "We connect to your Dubai server through a secure VPN with read-only credentials. Our system reads the data directly — the fastest and most accurate method.",
    pros: ["Real-time data — as fresh as it gets", "No installation on your server needed", "We manage everything remotely"],
    cons: ["Requires VPN credentials for our system", "Requires network access from our server to yours", "You need to be comfortable giving read-only access"],
    frequency: "Real-time (every 5-15 minutes)",
    effort: "Provide VPN credentials + read-only database user.",
  },
];
